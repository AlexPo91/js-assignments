...
function getRectangleString(width, height) {
    //throw new Error('Not implemented');
    let horizontLine = width-2;
    let vertikalLine = height -2;
    let topLine = `┌${'─'.repeat(horizontLine)}┐\n`
    let bottomLine = `└${'─'.repeat(horizontLine)}┘\n`
    let middleLine = `│${' '.repeat(horizontLine)}│\n`
    return topLine + middleLine.repeat(vertikalLine) +bottomLine
    }
...
return str.split('').map(i => string.indexOf(i) == -1 ? i :  rot13[string.indexOf(i)]).join('')
}
